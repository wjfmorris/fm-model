...
class GuidedAppTests(unittest.TestCase):
    def test_guided_demo_and_target_invalidation(self):
        ...
        self.assertFalse(any(s.label == 'Evidence threshold' for s in at.slider))
        self.assertFalse(any(s.label == 'Recruitment screening percentile' for s in at.slider))
        self.assertTrue(any(m.label == 'Evidence threshold' and m.value == '70%' for m in at.metric))
        initial_benchmark = next(m.value for m in at.metric if m.label == 'Recruitment benchmark')
        at.button(key='calculate_target').click().run()
        self.assertFalse(at.exception)
        self.assertEqual(at.session_state['target_scenario']['requested_probability'], 0.70)
        ...
        changed_benchmark = next(m.value for m in at.metric if m.label == 'Recruitment benchmark')
        self.assertNotEqual(initial_benchmark, changed_benchmark)
        self.assertFalse(at.exception)
