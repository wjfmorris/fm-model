...
class SquadPlanner:
    ...
    The current forecast starts from the latest observed team-process profile and,
    when historical player seasons are available, moves that profile from the prior
    XI to the current XI before forecasting next season. Transfer effects are then
    simulated by replacing the outgoing player's
    ...
    def plan(
        ...
        *,
        ...
        if not squad_adjustment_evidence.get("applied"):
            warnings.append(
                "The starting GF/GA forecast could not be fully adjusted from last season's XI to the current XI: "
                + squad_adjustment_evidence.get("reason", "historical squad evidence was unavailable")
            )
        ...
        return {
            ...
            "current_forecast": {
                **current,
                ...
                "evidence": (
                    "Goal-driver prediction starting from the latest observed team-process profile; "
                    "where historical player seasons are available, that profile is adjusted position-by-position "
                    "from the previous XI to the current modelled XI before forecasting."
                ),
