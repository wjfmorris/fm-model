...
3. Let the app test every eligible imported performance metric against the team outcomes relevant to that position, validate those relationships across clubs, and select the strongest non-redundant metrics for each position.
...
Season-target frontiers use a fixed **70% empirical evidence threshold**; this is a model standard rather than a user tuning control. Recruitment stat thresholds are then derived from the chosen target finish: the finishing rank is converted to its midpoint percentile within the league, and selected same-position metrics use that percentile as their benchmark. Position-change priorities are displayed from least important to most important.\n\nThe default recruitment focus is **learned from the uploaded save, not hard-coded by position**. Every eligible independent numeric performance field is tested only against role-relevant outcomes: ST and AM/W against scoring; GK and CB against preventing goals; FB/WB and CM/DM against both. Metric evidence uses leave-one-club-out validation; metrics that do not improve on a league-average baseline are not selected, and highly redundant metrics are pruned. The resulting screening thresholds still come from positional peer distributions. These are predictive associations, not causal proof or guaranteed transfer goal gains. Manual metric selection exists only as an explicit override. Missing costs remain unknown, and candidate uploads cannot move the reference benchmark.
